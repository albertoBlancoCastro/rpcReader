...
%
%2025-03-15 - Implementation of the dynamic tmp folder on DB distribution
...
if(isfield(conf,'DB'))
    if(conf.DB.active)
        ...
        RN                = num2str(floor(rand*10000));mkdirOS([conf.DB.tmpFolder RN b],OS,0);tmpFolder = [conf.DB.tmpFolder RN b];
    end
else
    conf.DB.active    = 0;
end
...
if strcmp(type,'time')
    ...
    if (conf.DB.active)
        file2Send         = [tmpFolder dev2Distribute '_' varName '.csv'];
        writeCSV4DB({conf,file2Send,dateRegularFormat,varName,var2Distribute,type,subType});
        sendData2DB({conf,remoteIP,user,pass,port,tmpFolder,systemName,dev2Distribute,varName,file2Send});
    end
