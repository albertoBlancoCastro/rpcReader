...
daq.subName                             = '3';
    ...
    daq.rAccess.remotePath              = ['/home/rpcuser/hlds/'];
    ...
    daq.unpacking.zipFiles              = 1;                               %keep data on origing and compress
    daq.unpacking.writeTDCCal           = 0;
    daq.unpacking.downScale             = 5;                               %1 is no downScale
...
ana.path                                    = [HOME 'ana' b];mkdirOS(ana.path,OS,1);
    ...
    ana.calibration.QPEDParam               = 'Offset_2024_07_18.mat';
    ana.calibration.LonYParam               = 'YOffSet_2024_07_17.mat';
    ...
    ana.calibration.longitudinalY.active    = 1;
    ...
    ana.param.strips.strips                 = 14 ;
    ana.param.strips.pitch                  = 18;%mm
    ana.param.strips.planes                 = 1;
    ana.param.strips.planesZPos             = [0 200 300 400];
    ana.param.strips.ActivePlanes           = [1 0 0 0];
    ...
    ana.report.address4Email                = {'alberto@coimbra.lip.pt';};

configuration.ana                       = ana;


%% Logs
log = initLog(); log.active        = 0; log.type          = 'criticallog'; log.localPath     = [SYS 'logs' b 'criticallog.log'];system(['touch ' log.localPath]);
configuration.logs = log;

log = initLog(); log.active        = 0; log.type          = 'syslog'; log.localPath     = [SYS 'logs' b 'syslog.log'];system(['touch ' log.localPath]);
configuration.logs(2) = log;

log = initLog(); log.active        = 0; log.type          = 'netlog'; log.localPath     = [SYS 'logs' b 'netlog.log'];system(['touch ' log.localPath]);
configuration.logs(3) = log;

%% Alarms
alarm = initAlarm(); alarm.system = SYSTEMNAME;alarm.active        = 0; alarm.type          = 'netAccess'; %alarm.TO     = {'alberto@coimbra.lip.pt';'alberto.blanco.abc@gmail.com';};
configuration.alarms = alarm;

alarm = initAlarm(); alarm.system = SYSTEMNAME;alarm.active        = 0; alarm.type          = 'unpacking'; %alarm.TO     = {'rpc.slow.control@gmail.com';};
...
alarm = initAlarm(); alarm.system = SYSTEMNAME;alarm.active        = 0; alarm.type          = 'Online Monitoring'; %alarm.TO     = {'rpc.slow.control@gmail.com'};
